...
const filterByCountry = (users, country) => {
  const newList = users.filter((users) => users.nat === country);
  return newList;
};

const filterByGender = (users, gender) => {
  const newList = users.filter((users) => users.name.title === gender);
  return newList;
};

const listEmails = (users) => {
  const newList = users.map((user) => user.email);
  return newList;
};

const reformatEmails = (users) => {
  const newList = users.map((user) => {
    const lastName = user.name.last;
    firstName = user.name.first;
    domain = `evilcorp.${user.nat.toLowerCase()}`;
